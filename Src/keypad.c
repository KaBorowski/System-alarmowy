...
/**
 * Read value of presses key
 *
 * @param GPIO_Pin Detected Column Pin
...
char KEYPAD_ReadKey(uint16_t GPIO_Pin) {
	...
	if (HAL_GPIO_ReadPin(KEYPAD_COLUMN_PORT, GPIO_Pin))
		return key;
...
/**
 * Reset variable, counting press amount
 */
