...
void ADMIN_UART_RxCpltCallback(){
	...
	if (rx_it == ADMIN_START1 && admin_receive_status != RECEIVING){
	...
	else if(rx_it == ADMIN_START2 && rx_index == 1 && rx_buffer[rx_index-1] == ADMIN_START1){
		...
		rx_buffer[rx_index] = rx_it;
		rx_index++;
		if(rx_index > (ADMIN_MIN_FRAME_LENGTH+ADMIN_MAX_DATA_SIZE))	rx_index=0;
	}else{
		rx_index = 0;
		memset(rx_buffer, 0, ADMIN_MIN_FRAME_LENGTH+ADMIN_MAX_DATA_SIZE);
	}
	if(rx_it == ADMIN_CRC && admin_receive_status == RECEIVING
				&& rx_index == ADMIN_CRC_OFFSET(rx_buffer[ADMIN_LENGTH_OFFSET])){
			rx_index = 0;
...
BoolType ADMIN_SendFrame(uint8_t command, uint8_t *data, uint8_t data_length){
	...
	if (data == NULL && data_length != 0)	data_length = 0;
	else if (data_length > ADMIN_MAX_DATA_SIZE)	data_length = ADMIN_MAX_DATA_SIZE;
	...
	HAL_UART_Transmit(huart_admin, frame_content, ADMIN_CRC_OFFSET(data_length)+1, 100);
